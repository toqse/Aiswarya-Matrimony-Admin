...
export default function EducationManagement() {
  ...
  const [deleteItem, setDeleteItem] = useState<EducationItem | null>(null);
  const [toggleItem, setToggleItem] = useState<EducationItem | null>(null);
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["master", "educations", search, page],
    queryFn: () => fetchAdminEducations({ search: search.trim() || undefined, page, page_size: 20 }),
  });
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["master", "educations"] });

  const saveMut = useMutation({
    mutationFn: async () => {
      if (!name.trim()) throw new Error("Name is required");
      if (editItem) return updateEducation(editItem.id, { name: name.trim() });
      return createEducation({ name: name.trim() });
    },
    onSuccess: () => {
  ...
  const toggleMut = useMutation({
    mutationFn: (id: number) => toggleEducationStatus(id),
    onSuccess: (data) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Education Management</h1>
          <p className="text-muted-foreground">Manage highest education options for profiles</p>
        </div>
        <Button
          onClick={() => {
            setEditItem(null);
