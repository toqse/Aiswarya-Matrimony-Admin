...
export default function OccupationManagement() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["master", "occupations", search, page],
    queryFn: () => fetchAdminOccupations({ search: search.trim() || undefined, page, page_size: 20 }),
  });
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["master", "occupations"] });

  const saveMut = useMutation({
    mutationFn: async () => {
      if (!name.trim()) throw new Error("Name is required");
      if (editItem) return updateOccupation(editItem.id, { name: name.trim() });
      return createOccupation({ name: name.trim() });
    },
    onSuccess: () => {
  ...
  const delMut = useMutation({
    mutationFn: (id: number) => deleteOccupation(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Occupation Management</h1>
          <p className="text-muted-foreground">Manage occupation options for profiles</p>
        </div>
        <Button
          onClick={() => {
            setEditItem(null);
        ...
        >
          <Plus className="h-4 w-4 mr-1" /> Add Occupation
        </Button>
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editItem ? "Edit" : "Add"} Occupation</DialogTitle>
          </DialogHeader>
